javascript:(function() {
  ...
  const url = 'http://{HOST}/capture?password={PASSWORD}&template=p&url=' + encodeURIComponent(location.href) + '&title=' + encodeURIComponent(title) + '&body=' + encodeURIComponent(body);
  console.log(url);
  // const oldUrl = location.href;
  location.href = url;
  // var checkBack;
  // checkBack = function {
  //   if (location.hrehf != oldUrl) {
  //     window.setTimeout(function() {location.href = oldUrl;}, 100);
  //   }
  // }
  //window.fetch(url).then(function() { console.log('saved ' + url)});
})()
